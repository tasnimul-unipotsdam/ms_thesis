...
def adjust_learning_rate(optimizer, epoch, batch_idx):
    ...
    lr_0 = base_lr
    M = 10
    T = 500 * num_batch  # total number of iterations
    ...
    def start_training(self, train_data, valid_data):
        ...
        for epoch in tqdm(range(1, self.params['epochs'])):
            ...
            for batch_idx, batch in enumerate(train_data):
                ...
                self.optimizer.zero_grad()  # 1
                lr = adjust_learning_rate(self.optimizer, epoch, batch_idx)
                output = self.model(inputs)
                loss = self.criterion(output, targets)
                loss.backward()  # 2
...
def train_model(model_name, experiment, epochs=500, base_lr=0.1, loss_type='BCE',
                optimizer='SGDM', init_type='normal', schedule_type=None, grad_clip=True,
                checkpoint=50):
