...
def seed_list():
    random_seed_list = [78, 80, 99, 21, 95, 85, 56, 98, 22, 40, 69, 88, 89, 39, 94, 71, 50, 60, 13, 86]
    random_seed = random_seed_list[16]
    return random_seed
...
def train_model(model_name, experiment, epochs=500, base_lr=0.0001, loss_type='CE',
    ...
    model = Unet3D(n_channels=1, n_classes=3, n_filters=32, drop=0.20, bilinear=True)
    model.to(device)
...
if __name__ == '__main__':
    train_model(experiment='model_17',
                model_name='en',
                epochs=2001,
                checkpoint=1000,
                optimizer='SGDM',
